...
$.extend(AutoSearch.prototype,{
        ...
        switch(self.option.tagsListStyle){
            ...
            case 'customize':
                if ($.isFunction(self.option.createTagsItemHandler)) {      
                    try{
                        ...
                        if(!html){
                            this._error('createTagsItemHandler返回的html错误');
                            return;
                        }
                    } catch(e) {
                    ...
                    this._error('createTagsItemHandler不是一个函数');
                    return;
                }
                self.elem.auto_input.before(html);
                break;
            case 'values':
                if(self.option.selectType == "single"){
                    ...
                    var title = self.elem.auto_input.val() ? self.elem.auto_input.val() + "," + data[self.option.data_attributes_title] : data[self.option.data_attributes_title];
            ...
            case 'other':
                if ($.isFunction(self.option.createOtherItemHandler)) {
                    try{
                        html = self.option.createOtherItemHandler(data,self.cache.cacheDate);
                        if(!html){
                            this._error('createOtherItemHandler返回的html错误');
                            return;
                        }
                    } catch(e) {
                    ...
                    this._error('createOtherItemHandler不是一个函数');
                    return;
                }
                break;                                    
            case 'default':
    ...
    _recommendElem: function(data) {        
        ...
        if($.isArray(data)){
            if(data.length == 0){
                html += '<div class="search-empty">找不到相关的内容！</div>';
            }else{
                    ...
                    case 'customize':
                        if ($.isFunction(self.option.createRecommendItemHandler)) {
                            try{
                                ...
                                if(!html){
                                    self._error('createRecommendItemHandler返回的html错误');
                                    return;
                                }
                            } catch(e) {
                            ...
                            self._error('createRecommendItemHandler不是一个函数');
                            return;
                        } 
                        break;      
                    case 'default':
